...
function rotateChange(idx = 0) {
    ...
    const org = [1, 0, 0, 0, 1, 0, 0, 0, 1];

    [0,1,2].forEach(idx => {
        let unitDeg = Math.PI / 180 * initVars.rotateAngle[idx]
        let trans = [1, 0, 0, 0, 1, 0, 0, 0, 1]
        if(idx === 0) {
          trans = [
            1, 0, 0, 
            0, Math.cos(unitDeg), Math.sin(unitDeg), 
            0, -Math.sin(unitDeg), Math.cos(unitDeg)
          ]
        }
        
        if(idx === 1) {
          trans = [
            Math.cos(unitDeg), 0, -Math.sin(unitDeg), 
            0, 1, 0, 
            Math.sin(unitDeg), 0, Math.cos(unitDeg)
          ]
        }
        if(idx === 2) {
          trans = [
            Math.cos(unitDeg), -Math.sin(unitDeg), 0,
            Math.sin(unitDeg), Math.cos(unitDeg), 0, 
            0, 0, 1
          ]
        }
        glMatrix.mat3.mul(org, glMatrix.mat3.fromValues(...org), glMatrix.mat3.fromValues(...trans))
    });
  
    initVars.rotateBase[0] = org.slice(0, 3)
    initVars.rotateBase[1] = org.slice(3, 6)
    initVars.rotateBase[2] = org.slice(6, 9)
